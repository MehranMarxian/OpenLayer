...
v0.11.0 built a screen that could tell you exactly which model files you were missing, where each one belonged, and how big it was — and then left you to go and fetch them. This release closes that gap for model files: the rows that say something is missing can now download it, one file at a time, each one confirmed individually before anything is fetched. The interesting part of the feature is what it refuses to do, and those refusals are listed below rather than hidden. Also here: a Flux.2 preset for the GGUF-quantised model, and the removal of two presets that had been advertising themselves as unfinished since v0.2.2.

### Added

- **The Setup screen can install a missing model.** A row that is missing a file grows an Install button, and pressing it opens a confirmation inside that row — not a dialog — naming the file, its size, the exact folder it will be written to, and **the host it comes from**. The host is shown deliberately: it is the only part of a download URL that says who is being trusted, and it is the part a long Hugging Face path hides. Only the Download button in that block sends anything. The work is done by ComfyUI-Manager, which must already be installed; rows offer nothing they cannot deliver, so the button appears only when the Manager has answered a version probe.
- **Success is claimed from re-reading ComfyUI's inventory, not from the downloader.** ComfyUI-Manager reporting that it finished a queue item and ComfyUI being able to see a usable model in the folder its loader reads are different facts. A file that downloaded into a folder ComfyUI does not read satisfies the first and fails you, so the screen re-runs the setup check and says "installed and ComfyUI can see it" only when that check agrees.
- **OpenLayer refuses to install several things, and says why in each row.** A **licence-gated** model is refused because an unauthenticated download of a gated URL saves an HTML error page under the model's filename, which is then indistinguishable from a corrupt model until ComfyUI tries to load it. A model that is **already on disk in the wrong folder** is refused because the file is there and the operation needed is a move, which OpenLayer cannot perform. **Custom node packages** are refused wholesale in this version and keep Copy Link — ComfyUI-Manager's git-URL route needs a non-default flag, and its queue route needs registry fields this project does not carry.
- **It will not start a queue that already has work in it.** ComfyUI-Manager's queue is shared with its own web interface, so starting one that already holds items would run someone else's downloads for them. Every request OpenLayer makes carries its own identifier so queue messages can be attributed rather than assumed. There is no queue reset and no server reboot at any layer.
...
- **Assisted install covers model files only.** Custom node packages still have to be installed through ComfyUI-Manager yourself, and every node row keeps Copy Link.
- **It needs ComfyUI-Manager.** With no Manager installed, the Setup screen behaves exactly as it did in v0.11.0 — every name, folder, size and link, and no Install buttons.
- **Licence-gated models cannot be installed this way** and say so on their row. The two Flux.1 weights and Flux.2's model and encoder are gated; accept the licence in a browser and download them by hand.
- **A download keeps going if you close the panel.** ComfyUI-Manager owns it, and that is correct — what stops when the panel closes is OpenLayer asking about it, not the download.
- **The Flux.2 GGUF preset is slow on a 12 GB card, by a wide margin.** An 18.7 GB quantised model plus a 16.8 GB text encoder means ComfyUI streams most of it from system RAM: minutes per image, not seconds. It also needs `mistral_3_small_flux2_fp8.safetensors`, which is licence-gated and therefore one of the files assisted install will not fetch for you.
