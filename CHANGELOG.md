...
- **Installation still requires the UXP Developer Tool, and the `.ccx` on the release page is an open question rather than a supported path.** It was not part of the tagged code: the tooling that generates it landed in `ec1f2f5` the day after the tag, and the file was attached to this release on 2026-08-02 rather than reissued as its own version, because it changes no plugin behaviour. Whether a double-click install works on a machine that has never had developer mode enabled is still unanswered — that needs a clean second machine, and it is the one thing this release actually asks for. Reasoning and what would count as a pass are in `docs/DISTRIBUTION_SPIKE.md`.
