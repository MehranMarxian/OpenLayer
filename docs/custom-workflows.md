...
`inpaint-flux2-klein` is the first inpaint preset that is not built on Flux Fill. It loads the FLUX.2 Klein 4B stack -- `UNETLoader`, `CLIPLoader` with type `flux2`, `VAELoader`, `ModelSamplingAuraFlow` -- and inpaints with core ComfyUI nodes: `SetLatentNoiseMask` on the `VAEEncode` of the crop, plus `ReferenceLatent` on both the positive and the negative conditioning. It shares `InpaintCropImproved` and `InpaintStitchImproved` with `inpaint-flux-fill-cropstitch`, and that pair is required rather than optional -- see `docs/inpainting.md` for the measurements. Like the Flux Fill presets it reads the mask from the source PNG's alpha channel, so it has no `maskImage` injection target; unlike them it takes the panel's steps, CFG and denoise rather than locked reference defaults.
