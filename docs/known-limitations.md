...
- `txt2img-z-image-turbo` workflow generation
- `img2img-basic` workflow generation foundation
- `img2img-z-image-turbo` workflow generation foundation
- `sketch2img-linecn-basic` Sketch to Image generation foundation
- `prompt-from-layer-florence2` text workflow with Task and Num beams controls
- `upscale-basic` pixel/model upscale workflow
...
- **Qwen-Image 2.1 is research-licensed.** Its three presets (Text to Image, Image to Image, Multi-Reference) use weights Qwen licenses for research and evaluation only; commercial use needs a separate licence from Qwen. Each preset carries "(research licence)" in its name, none is a default, and Setup gives you the links but never downloads the files itself. They also need ComfyUI 0.37.0 or newer; an older ComfyUI reports `TextEncodeQwenImage21` as a missing node.
- **Qwen-Image 2.1 edits repaint the whole frame.** Objects stay within a pixel of where they were, but untouched areas can come back slightly darker — 6 to 12 levels, measured on photographs. Compare before you keep an edit. Editing *only a selection* is in Edit Image since v0.36 (Capture Selection): the edit is colour-matched from the untouched ring around the selection, which took the smooth-sky edge that cut it from v0.35 down to an invisible −0.47 levels. It needs the Qwen-Image 2.1 preset, and so do reference layers inside an edit (v0.37, up to four; each adds about 30 s).
- **Qwen-Image 2.1 does not replace Remove Background.** Its own background-removal prompt worked on the vendor's sample image and returned an empty, fully transparent frame on a real photograph of a person in a street, in four runs out of four across two text encoders. Remove Background stays on BiRefNet.
- **Qwen-Image 2.1 lettering is strong, not guaranteed.** Clean type came back spelled exactly every time it was tested, at 1024² and 2048²; stylised hand-painted lettering on a wooden sign came back misspelled. At 2048² the model also runs slightly off its own sampling schedule until ComfyUI fixes [#16447](https://github.com/comfy-org/ComfyUI/issues/16447).
- **Transparent Background only exists where the model can do it.** The toggle appears for Qwen-Image 2.1 alone. The same model keeps a cut-out layer a cut-out when you edit it in Image to Image; every other edit preset returns an ordinary opaque layer from a transparent one, as before.
- **Remove Background gives you a layer with alpha, not a layer mask.** The cutout arrives as its own layer at the resolution you captured, carrying your original pixels unchanged with an alpha channel added — measured at a mean absolute difference of 0.0 against the source, so nothing is re-rendered. What it does not do is put a mask on the layer you started from, which would be the fully non-destructive version. Hide or keep the original as you prefer; the cutout does not touch it.
- **Unflatten needs a subject standing in front of a background.** A close-up that fills the frame has no front and back to find: it comes back with the picture unchanged on the background layer and empty layers above it. This is true of photographs and generated images alike — it is about the composition, not the source. **Since v0.21 the panel notices and says so**, instead of reporting a run that changed nothing as a successful import. This was previously listed here as undetectable, and for the reasons given it was: nothing in a UXP panel can decode a PNG, and two cheap proxies were measured and both failed. What changed is that the blank-plate work in v0.20 already reads each placed layer's alpha through Photoshop's imaging API, so the question stopped being "what is in this plate" and became "how many survived" — which is a count. The limit itself has not moved, and is still stated on the screen and in the tool description an agent reads.
...
- **Multi-Reference Composition with FLUX.2 Klein does not preserve a specific person's likeness.** It carries clothing, props, setting and lighting from a reference; a person's face comes back as a plausible stranger rather than the person photographed, confirmed against four real photographs and dozens of Klein-generated sources. Treat it as scene composition, not as a way to put someone recognisable into a picture. The Qwen-Image 2.1 preset kept a generated person's hair and clothing, but has not been tested with a real photograph of a face, so do not assume likeness from it either.
- **The reference list is capped at 8 with Klein and 6 with Qwen-Image 2.1** — Qwen's bound is time (about 30 seconds per reference on a 12 GB card; six took 179 s), Klein's a sanity bound rather than a measured quality limit — testing found no reference count at which composition degraded, up to 6. Reference order matters more than count: an object that has to sit behind the other subjects is more likely to render cleanly if it comes earlier in the list.
...
- Inpaint has four presets: `inpaint-basic` (SD 1.x), `inpaint-flux-fill-basic` (Flux Fill), `inpaint-flux-fill-cropstitch` (Flux Fill with crop-and-stitch), and `inpaint-flux2-klein` (FLUX.2 Klein with crop-and-stitch). The crop-and-stitch presets require lquesada's `comfyui-inpaint-cropandstitch` custom-node package; without it the preset is unavailable and the original Flux Fill preset still works.
...
- Z_image_Turbo presets use `UNETLoader`, `CLIPLoader`, and `VAELoader` instead of the checkpoint loader.
- `txt2img-flux1-dev-fp8` is a checkpoint-style Flux Text to Image preset for `flux1-dev-fp8.safetensors`.
- FLUX.2 Klein presets (`txt2img-flux2-klein`, `img2img-flux2-klein`, `edit-flux2-klein`, `inpaint-flux2-klein`, `multi-reference-flux2-klein`) use `flux-2-klein-4b-fp8.safetensors` (4.07 GB), `qwen_3_4b.safetensors` (8 GB, shared with Z_image_Turbo), and `flux2-vae.safetensors` (336 MB). Klein is Apache-2.0 and ungated. `edit-flux2-klein` is structurally different from image-to-image: it uses `ReferenceLatent` conditioning at denoise 1, not a starting latent at partial denoise.
...
- The setup pack contains no model weights. Every registered preset's files come to roughly 192 GB deduplicated, and seven of them are licence-restricted, so it ships the requirements list and a downloader instead. An internet connection is required.
...
- The Preview panel's Import button covers the generation tools that publish a result to it — before v0.35 it was enabled but did nothing for Multi-Reference and Unflatten. Remove Background and Layer Maps import from their own screens, and Live Painting's "Import Refined as Layer" stays on the dashboard.
